...
namespace NetFx40ConsoleTest
{
    class Program
    {
        ...
        static IntPtr[] handles = new IntPtr[] { handle1, handle2, /*handle3*/ };

        static void Main(string[] args)
        {
            // Do doWaitForMultipleObjects
            DoWaitForMultipleObjects(handles);
        }

        static void DoWaitForMultipleObjects(IntPtr[] handles)
        {
            // Wait for any of the handles to become signaled
            int result = WaitForMultipleObjects(handles.Length, handles, false, INFINITE);
        ...
        static IntPtr CreateWork2()
        {
            Thread thread2 = new Thread(() =>
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    // add handle3
                    List<IntPtr> handleList = new List<IntPtr>(handles);
                    DoWaitForMultipleObjects(handleList.ToArray());
                }
            });
            thread2.Start();
